...
def telugu_to_prompt(s):
    ...
    for k,v in mapping.items():
        if k in s:
            eng=v
            break
    return f"{eng}, ultra realistic 8K, vibrant colors, sharp focus, cinematic lighting, photorealistic"
...
if voice_mode == "Upload My Voice File":
    uploaded_voice=st.file_uploader("Upload MP3/WAV/M4A", type=["mp3","wav","m4a","ogg"])
    if uploaded_voice:
        st.audio(uploaded_voice)

st.markdown("### BGM - WITH UPLOAD")
bgm_mode = st.radio("BGM:", ["Auto BGM", "Upload My BGM", "No BGM"], horizontal=True)
uploaded_bgm=None
if bgm_mode == "Upload My BGM":
    uploaded_bgm=st.file_uploader("Upload BGM MP3/WAV", type=["mp3","wav","m4a"], key="bgm")
    if uploaded_bgm:
...
if st.button("GENERATE PA1 VIDEO", use_container_width=True):
    if not script:
        ...
        status=st.empty()
        timer=st.empty()
        st.info(f"PA1 Project: {len(sents)} scenes")
        ...
        for i,s in enumerate(sents):
            ...
            status.markdown(f"PA1 Scene {i+1}/{len(sents)}")
            timer.markdown(f"Elapsed: {int(elapsed)}s | Remaining: {int(eta)}s")
            try:
                prompt=telugu_to_prompt(s)
                url=f"https://image.pollinations.ai/prompt/{requests.utils.quote(prompt)}?width=1280&height=720&model=flux&enhance=true&nologo=true&seed={i*101}"
                r=requests.get(url,timeout=90)
                im=Image.open(io.BytesIO(r.content)).convert("RGB")
                im=ImageEnhance.Sharpness(im).enhance(2.2)
                im=ImageEnhance.Color(im).enhance(1.4)
                im=add_pa1_watermark(im)
                st.image(im, caption=f"PA1 Scene {i+1}", use_container_width=True)
                imgs.append(im)
            except:
                pass
        ...
        if voice_mode == "Upload My Voice File" and uploaded_voice:
            ...
            if bgm_mode != "No BGM":
                try:
                    if uploaded_bgm:
                        with open("/tmp/my_bgm.mp3","wb") as f:
                            f.write(uploaded_bgm.getbuffer())
                        bgm=AudioSegment.from_file("/tmp/my_bgm.mp3") - 18
                    else:
                        br=requests.get("https://cdn.pixabay.com/audio/2022/06/07/audio_b9bd4170e8.mp3", timeout=15)
                        open("/tmp/bgm.mp3","wb").write(br.content)
                        bgm=AudioSegment.from_file("/tmp/bgm.mp3") - 18
                    bgm_loop=(bgm * (int(len(combined)/len(bgm))+1))[:len(combined)]
                    combined=bgm_loop.overlay(combined)
                except:
                    pass
        ...
        if imgs:
            ...
            for img in imgs:
                ...
                for z in range(24):
                    ...
                    zoomed=add_pa1_watermark(zoomed)
            ...
            try:
                ...
                if final_audio_path and os.path.exists(final_audio_path):
                    ...
                    out_path="/tmp/PA1_PavanMynam_Final.mp4"
                    final.write_videofile(out_path, codec='libx264', audio_codec='aac')
                    st.video(out_path)
                    st.download_button("DOWNLOAD PA1 VIDEO - PAVAN MYNAM", open(out_path,"rb"), "PA1_PavanMynam_Official.mp4", use_container_width=True)
                else:
                    st.video(video_path)
                    st.download_button("DOWNLOAD PA1 VIDEO", open(video_path,"rb"), "PA1_PavanMynam.mp4")
            except:
                st.video(video_path)
                if final_audio_path:
                    st.audio(final_audio_path)
