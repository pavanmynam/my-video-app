...
def telugu_to_prompt(s):
    mapping={"గ్రామం":"old Indian village mud houses","ఇల్లు":"traditional mud house","అమ్మాయి":"beautiful young Indian girl in saree","అబ్బాయి":"young Indian man","ఏడుపు":"crying emotional closeup","ప్రేమ":"romantic soft light","రాత్రి":"night moonlight cinematic","అడవి":"dense forest misty","నది":"river beautiful","పొలం":"green paddy fields"}
    eng=s
    for k,v in mapping.items():
        if k in s:
            eng=v
            break
    return f"{eng}, ultra realistic 8K, vibrant colors, sharp focus, cinematic lighting, photorealistic, detailed"
...
script=st.text_area("INPUT - TELUGU SCRIPT", height=130, placeholder="పాత గ్రామం, మట్టి ఇల్లు, ఒక అమ్మాయి ఏడుస్తోంది...")
...
if st.button("GENERATE PA1 VIDEO", use_container_width=True):
    if not script:
        ...
        status=st.empty()
        timer=st.empty()
        st.info(f"PA1 Project: {len(sents)} scenes | Final video ~ {len(sents)*2} sec")
        ...
        for i,s in enumerate(sents):
            ...
            status.markdown(f"PA1 Scene {i+1}/{len(sents)} generating...")
